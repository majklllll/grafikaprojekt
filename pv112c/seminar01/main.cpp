...
void init() {
  ...
  GLuint color_location = glGetAttribLocation(program, "color");
  ...
  // Define the format of the buffered data (two floats, normalized)
  glVertexAttribPointer(color_location, // index/location
                        3,                 // number of components (1,2,3,4)
                        GL_UNSIGNED_BYTE,   // data type of component
                        GL_TRUE,          // should components be normalized?
                        0,                 // stride between attributes
                                           // 0 = tightly packed
                        0                  // offset into the buffer
                        );
